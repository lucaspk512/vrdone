vidvrd_categories = [
    ...
    'watercraft',
    'whale',
    'zebra',  # 前面几行的顺序和imagenet-DET中的顺序相同
    'ball',
    'frisbee',
    'sofa',
    'skateboard',
    'person',
]  # 后面是不在 imageNet-DET中的类

vidvrd_category_id_to_name = {idx: cat for idx, cat in enumerate(vidvrd_categories)}
vidvrd_category_name_to_id = {cat: idx for idx, cat in enumerate(vidvrd_categories)}

vidvrd_pred_categories = [
    "__background__",
    "taller",
    "swim_behind",
    "walk_away",
...
vidvrd_pred_id_to_name = {idx: cat for idx, cat in enumerate(vidvrd_pred_categories)}
vidvrd_pred_name_to_id = {cat: idx for idx, cat in enumerate(vidvrd_pred_categories)}
...
vidor_category_id_to_name = {v["id"]: v["name"] for v in vidor_categories}
vidor_category_name_to_id = {v["name"]: v["id"] for v in vidor_categories}

vidor_pred_id_to_name = {idx: cat for idx, cat in enumerate(vidor_pred_categories)}
vidor_pred_name_to_id = {cat: idx for idx, cat in enumerate(vidor_pred_categories)}

vidor_category_id_to_fg = {v["id"]: "FG" for v in vidor_categories}
vidor_pred_id_to_fg = {idx: "FG" for idx, cat in enumerate(vidor_pred_categories)}
