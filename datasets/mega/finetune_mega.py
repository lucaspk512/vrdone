...
def train(cfg, local_rank, distributed, lr_rate):
    ...
    ## update checkpointer lr
    for param_group in checkpointer.optimizer.param_groups:
        param_group['lr'] *= lr_rate
    
    for idx in range(len(checkpointer.scheduler.base_lrs)):
        checkpointer.scheduler.base_lrs[idx] *= lr_rate
...
def main():
    ...
    ### customized cfg
    parser.add_argument("--lr_rate", type=float, default=0.1)
    parser.add_argument("--max_iter", type=int, default=210000)
    ### 
    ...
    model = train(cfg, args.local_rank, args.distributed, args.lr_rate)
