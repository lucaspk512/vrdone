...
class VidVRDDataset(torch.utils.data.Dataset):
    # cattle = cow, domestic_cat = cat, watercraft = boat,
    # ball = sports ball, sofa = coach
    ...
    def __init__(self, image_set, data_dir, img_dir, anno_path, img_index, transforms, is_train=True):
        ...
        self._img_dir = os.path.join(self.img_dir, "%s.jpg")   # i.e., 对于视频的训练数据，是把视频提取为JPEG图片再做的
        self._anno_path = os.path.join(self.anno_path, "%s.json") # vidvrd-dataset/train/%s.json
        ...
        print("Num of frames: {}".format(len(self)))
    ...
    def _get_train(self, idx): # modified, #已经改好了
        ...
        filename = "{}/{}_{:06d}".format(video_name, video_name, frame_id)
        ...
        for i in range(len(offsets)):
            ...
            ref_video_name, ref_frame_id = ref_filename.split('/')
            ref_frame_id = int(ref_frame_id) + 1
            ref_filename = "{}/{}_{:06d}".format(ref_video_name, ref_video_name, ref_frame_id)
    ...
    def _get_test(self, idx): # 已好
        print("get test called once")
        filename = self.image_set_index[idx]
        video_name, frame_id = filename.split('/')
        frame_id = int(frame_id)
        filename = "{}/{}_{:06d}".format(video_name, video_name, frame_id + 1)
        ...
        ref_filename = "{}/{}_{:06d}".format(ref_video_name, ref_video_name, ref_frame_id + 1)
        img_ref = Image.open(self._img_dir % ref_filename).convert("RGB")
        img_refs_l.append(img_ref)
        ...
        if cfg.MODEL.VID.MEGA.GLOBAL.ENABLE:
            ...
            for id in range(size):
                ...
                video_name_g, frame_id_g = filename_g.split('/')
                frame_id_g = int(frame_id_g)
                filename_g = "{}/{}_{:06d}".format(video_name_g, video_name_g, frame_id_g+1)
    ...
    def filter_annotation(self):  # 已改好
        ...
        for idx in range(len(self)):
            ...
            video_frame = self.image_set_index[idx].split('/')  # e.g., ILSVRC2015_train_00005004/000010 idx 的范围是所有视频的帧的总数
            video_name = video_frame[0]                         # e.g., ILSVRC2015_train_00005004
            frame_id = int(video_frame[1])                      # e.g., 10
            with open(self._anno_path % video_name,'r') as json_file:
    ...
    def map_class_id_to_class_name(class_id):
        return VidVRDDataset.classes[class_id]
