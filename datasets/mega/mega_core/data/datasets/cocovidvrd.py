...
class COCOVidVRDDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, image_set, data_dir, img_dir, anno_path, transforms, is_train=True):
        ...
        ## process synonyms 
        synonyms_vidvrd2coco = img_anno_info["synonyms_vidvrd2coco"]
        for k in self.cocoCatId2CatName.keys():
            for vidvrd_name, coco_name in synonyms_vidvrd2coco.items(): # e.g., "ball/sports_ball":"sports ball",
                if self.cocoCatId2CatName[k] == coco_name:
                    self.cocoCatId2CatName[k] = vidvrd_name
                    break
