...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, 
                 act_layer='relu', drop=0.0, with_bias=True):
        super().__init__()
        self.num_layers = num_layers
        h = [hidden_dim] * (num_layers - 1)
        self.act_layer = _get_activation_fn(act_layer)
        self.layers = nn.ModuleList(
            nn.Linear(n, k, bias=with_bias) for n, k in zip([input_dim] + h, h + [output_dim])
        )
    ...
    def forward(self, x):
        for i, layer in enumerate(self.layers):
            x = self.act_layer(layer(x)) if i < self.num_layers - 1 else layer(x)
            x = self.drop(x)
        return x
...
class ConvMLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, kernel_size=1,
                 act_layer='gelu', drop=0.0, with_bias=True):
        super().__init__()
        self.num_layers = num_layers
        h = [hidden_dim] * (num_layers - 1)
        self.act_layer = _get_activation_fn(act_layer)
        self.layers = nn.ModuleList(
            nn.Conv1d(n, k, kernel_size, padding=kernel_size//2, bias=with_bias) 
...
class MaskedConv1D(nn.Module):
    ...
    def forward(self, x, mask, downsample=True):
        ...
        # compute the mask
        if self.stride > 1 and downsample:
